...
func _physics_process(delta):
	...
	if Input.is_action_pressed("crouch"):
		...
		pcap.shape.height += crouch_speed * delta
	pcap.shape.height =  clamp(pcap.shape.height, crouch_height,d_height)
	# the sound and walk animation.
	if Input.is_action_pressed("w"):
		$Head/Camera3D/AnimationPlayer.play("walk")
		if $Head/Camera3D/Timer.time_left <=0:
			$Head/Camera3D/AudioStreamPlayer.pitch_scale = randf_range(1.1,2)
			$Head/Camera3D/AudioStreamPlayer.play()
			$Head/Camera3D/Timer.start(0.3)
